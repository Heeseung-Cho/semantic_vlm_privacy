#!/usr/bin/env bash
# SUBMISSION / Row 3 (+ FAM only): catonly (no cue) + FAM expansion + caption
set -euo pipefail
PROJECT_ROOT="${PROJECT_ROOT:-/home/choheeseung/workspace/vlm-privacy}"
REPO_ROOT="${PROJECT_ROOT}/challenge_repo"
CODE_DIR="${REPO_ROOT}"
ROW_DIR="${REPO_ROOT}/runs/main_ablation_2x2_submission/row3_fam_only"
...
mkdir -p "${ROW_DIR}/stage1_catonly" "${ROW_DIR}/stage1_catonly_fam" "${ROW_DIR}/stage2" "${ROW_DIR}/stage3"

python - <<PYEOF
import json
from pathlib import Path
src = Path("${BASE_STAGE1}"); dst = Path("${ROW_DIR}/stage1_catonly/stage1_semantic.json")
data = json.loads(src.read_text())
recs = data if isinstance(data, list) else data.get('records', [])
for r in recs:
    cats = set(r.get('semantic_categories', []) or [])
    r['proposal_prompts'] = [p for p in (r.get('proposal_prompts', []) or []) if p in cats]
if isinstance(data, dict): data['records'] = recs
dst.write_text(json.dumps(data, indent=2))
print(f'cat-only stage1 written: {dst}')
PYEOF

python "${CODE_DIR}/scripts/expand_stage1_to_family.py" \
  --input_path "${ROW_DIR}/stage1_catonly/stage1_semantic.json" \
  --output_path "${ROW_DIR}/stage1_catonly_fam/stage1_semantic.json" \
  --family_config "${CODE_DIR}/config/family_category_route4_v1.json"

python "${CODE_DIR}/semantic/run_stage2_detection.py" \
  --stage1_path "${ROW_DIR}/stage1_catonly_fam/stage1_semantic.json" \
