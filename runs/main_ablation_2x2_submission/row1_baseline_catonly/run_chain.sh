#!/usr/bin/env bash
# SUBMISSION / Row 1 (Baseline): catonly (no cue) + no FAM + caption
set -euo pipefail
PROJECT_ROOT="${PROJECT_ROOT:-/home/choheeseung/workspace/vlm-privacy}"
REPO_ROOT="${PROJECT_ROOT}/challenge_repo"
CODE_DIR="${REPO_ROOT}"
ROW_DIR="${REPO_ROOT}/runs/main_ablation_2x2_submission/row1_baseline_catonly"
...
mkdir -p "${ROW_DIR}/stage1_catonly" "${ROW_DIR}/stage2" "${ROW_DIR}/stage3"
...
python "${CODE_DIR}/semantic/run_stage2_detection.py" \
  --stage1_path "${ROW_DIR}/stage1_catonly/stage1_semantic.json" \
